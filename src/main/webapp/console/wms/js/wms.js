    ...
    if(window.ActiveXObject){
      ...
      return list;
        ...
        for(var i = 0; i < res.length; ++i){
        	if(res[i].firstChild != null){
        		list[i] = res[i].firstChild.nodeValue;
        	}
        }
        return list;
    }
...
//      return OpenLayers.Request.GET({url: url, async: false, params: {request: 'GetCapabilities', service: 'WMS', version: '1.1.1'}}).responseXML
      return OpenLayers.Request.GET({url: url, async: false, params: {request: 'GetCapabilities', service: 'WMS', version: '1.3.0'}}).responseXML
    }
...
//	    var xml = OpenLayers.Request.GET({url: url, async: false, params: {request: 'GetCapabilities', service: 'WMS', version: '1.1.1'}}).responseText
	    var xml = OpenLayers.Request.GET({url: url, async: false, params: {request: 'GetCapabilities', service: 'WMS', version: '1.3.0'}}).responseText
	    doc.loadXML(xml)
	    return doc
    } else if(/msie|trident/i.test(navigator.userAgent)){
    	// IE11
