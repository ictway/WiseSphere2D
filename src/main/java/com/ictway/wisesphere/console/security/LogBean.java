/*----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
package com.ictway.wisesphere.console.security;
...
import org.deegree.console.security.PasswordFile;
import org.deegree.console.security.SaltedPassword;
...
//        		 FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "Current password is blank.", null );
        		 FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "현재 비밀번호가 비어 있습니다.", null );
...
//       		 	FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "New password is blank.", null );
       		 	FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "새 비밀번호가 비어 있습니다.", null );
                FacesContext.getCurrentInstance().addMessage( null, fm );
                return CHANGE_PASSWORD;
        	}
...
//                FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "Current password is incorrect.", null );
                FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "현재 비밀번호가 맞지 않습니다.", null );
                FacesContext.getCurrentInstance().addMessage( null, fm );
                return CHANGE_PASSWORD;
            }
            if ( !newPasswordsMatch() ) {
//                FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "New passwords don't match.", null );
                FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "새 비밀번호가 일치하지 않습니다.", null );
                FacesContext.getCurrentInstance().addMessage( null, fm );
                return CHANGE_PASSWORD;
            }
...
//            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "Error updating password: " + e.getMessage(), null );
            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "비밀번호 수정 중 오류가 발생하였습니다(오류내용: " + e.getMessage()+")", null );
            FacesContext.getCurrentInstance().addMessage( null, fm );
            return CHANGE_PASSWORD;
        }

//        FacesMessage fm = new FacesMessage( FacesMessage.SEVERITY_INFO, "Password changed successfully.", null );
        FacesMessage fm = new FacesMessage( FacesMessage.SEVERITY_INFO, "비밀번호가 성공적으로 변경되었습니다.", null );
        FacesContext.getCurrentInstance().addMessage( null, fm );
        return CHANGE_PASSWORD;
    }
