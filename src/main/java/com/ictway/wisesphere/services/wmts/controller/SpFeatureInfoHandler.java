 ...
 */
class SpFeatureInfoHandler {

    private FeatureInfoManager featureInfoManager;

    private Map<String, TileLayer> layers;

    SpFeatureInfoHandler( FeatureInfoFormatsType conf, XMLAdapter controllerConf, DeegreeWorkspace workspace,
                        List<Theme> themes ) throws ResourceInitException {
        featureInfoManager = SpFeatureInfoManagerBuilder.buildFeatureInfoManager( conf, controllerConf, workspace );
    ...
    void getFeatureInfo( Map<String, String> map, HttpResponseBuffer response )
        ...
        SpFeatureInfoFetcher fetcher = new SpFeatureInfoFetcher( l, gfi );
