...
import static com.ictway.wisesphere.services.wmts.SpWMTSProvider.IMPLEMENTATION_METADATA;
 ...
 */
public class SpWMTSController extends AbstractOWS {

    private static final Logger LOG = getLogger( SpWMTSController.class );

    private String metadataUrlTemplate;

    private SpWmtsRequestDispatcher dispatcher;

    /**
     * @param configURL
     * @param serviceInfo
     */
    public SpWMTSController( URL configURL, ImplementationMetadata<?> serviceInfo ) {
    ...
    public void init( DeegreeServicesMetadataType serviceMetadata, DeegreeServiceControllerType mainConfig,
        ...
        SpWmtsBuilder builder = new SpWmtsBuilder( controllerConf, workspace );

        this.metadataUrlTemplate = builder.getMetadataUrlTemplate();

        dispatcher = new SpWmtsRequestDispatcher( controllerConf, mainMetadataConf, workspace, builder, getId() );
    }
