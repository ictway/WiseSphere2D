 ...
 */
class SpLockFeatureHandler {

    private static final Logger LOG = LoggerFactory.getLogger( SpLockFeatureHandler.class );
     ...
     * 
     */
    SpLockFeatureHandler( SpWebFeatureService master ) {
        this.master = master;
    }
    ...
    private Lock acquireLock( LockFeature request, LockManager manager, long expiryInMilliseconds )
        ...
        try {
            SpQueryAnalyzer queryAnalyzer = new SpQueryAnalyzer( request.getQueries(), master, master.getStoreManager(),
