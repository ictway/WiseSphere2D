...
public class Java2DRenderer implements Renderer {
    ...
    public void render( final PointStyling styling, final Geometry geom, String buff ) {
        if ( geom == null ) {
            LOG.debug( "Trying to render null geometry." );
            return;
        }
        ...
        renderGeometry = renderGeometry.getBuffer(new Measure(buff, "meter"));
        // TODO properly convert'em
        if ( renderGeometry instanceof Surface ) {
            rendererContext.polygonRenderer.render( styling, (Surface) renderGeometry );
        } else if ( renderGeometry instanceof Curve ) {
    ...
    public void render( final LineStyling styling, final Geometry geom, String buff ) {
        if ( geom == null ) {
            LOG.debug( "Trying to render null geometry." );
            return;
        }
    ...
    public void render( final PolygonStyling styling, final Geometry geom, String buff ) {
        if ( geom == null ) {
            LOG.debug( "Trying to render null geometry." );
            return;
        }
    ...
    public Geometry simplifyGeometry(Geometry geometry, double tolerance) {
    	...
    	AbstractDefaultGeometry temp = new DefaultPoint( null, null, null, new double[] { 0.0, 0.0 } );
    	
    	try {
    		com.vividsolutions.jts.geom.Geometry jg = ((AbstractDefaultGeometry)geometry).getJTSGeometry();
    		ret = TopologyPreservingSimplifier.simplify(jg, tolerance);
    		
    		retGeom = temp.createFromJTS(ret, crs);
    	} catch (Exception e) {
    		return geometry;
    	}
    ...
    public void render( Styling styling, Collection<Geometry> geoms ) {
        for ( Geometry geom : geoms ) {
            ...
            if ( styling instanceof PolygonStyling ) {
            	// ksjang
            	//geom = simplifyGeometry(geom, 100.0);
                render( (PolygonStyling) styling, geom );
            }
        }
    }
    ...
    @Override
    public void render( Styling styling, Collection<Geometry> geoms, String buff ) {
        for ( Geometry geom : geoms ) {
            if ( geom instanceof GeometryReference<?> ) {
                render( styling, ( (GeometryReference<?>) geom ).getReferencedObject() );
            }
            
            PolygonStyling style = new PolygonStyling();
            style.stroke = new Stroke();
            style.stroke.width = 1.0;
            style.stroke.color = Color.black;
            style.fill = new Fill();
            style.fill.color = new Color( 0x800000ff, true );
            
            //geom = geom.getBuffer(new Measure(buff, "meter"));
            
            if ( styling instanceof PointStyling ) {
            	//float opaicty = (float)((PointStyling)styling).graphic.opacity * 255;
            	//float opaicty = (float)((PointStyling)styling).graphic.opacity;
            	style.fill.color = ((PointStyling)styling).graphic.mark.fill.color;
            }
            if ( styling instanceof LineStyling ) {
                //render( (LineStyling) styling, geom, buff);
            	style.fill.color = ((LineStyling)styling).stroke.color;
            }
            
            ///if ( styling instanceof PolygonStyling ) {
                render( (PolygonStyling) style, geom, buff);
            //}
        }
    }
