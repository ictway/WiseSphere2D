...
//            FacesMessage fm = new FacesMessage( SEVERITY_INFO, "Executed " + sqlStatements.length + " statements successfully.", null );
            FacesMessage fm = new FacesMessage( SEVERITY_INFO, sqlStatements.length + "개의 쿼리를 성공적으로 실행했습니다.", null );
...
//            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "Error: " + t.getMessage(), null );
            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "데이터 처리 중 오류가 발생하였습니다(오류내용: " + t.getMessage() + ")", null );
            FacesContext.getCurrentInstance().addMessage( null, fm );
            return null;
        }
        return backOutcome;
