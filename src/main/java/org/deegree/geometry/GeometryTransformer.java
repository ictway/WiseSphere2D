...
public class GeometryTransformer extends Transformer {
    ...
    private <T extends Geometry> T transform( T geo, Transformation trans, Envelope domainOfValidity )
                            throws TransformationException, IllegalArgumentException {
    	//@@ksjang
    	String code = trans.getTargetCRS().getCodes()[0].getCode();
    	
    	
        //@@ksjangif ( TransformationFactory.isIdentity( trans ) ) {
    	if ( TransformationFactory.isIdentity( trans ) && !code.equals("2097")) {
            return geo;
        }
    ...
    private Points transform( Points points, Transformation trans )
        ...
        for ( Point point : points ) {
            ...
            /**@@
            try {
            	code = trans.getCodes()[0].getCode();
            	
            	if (code.equals("2097")) {
                	//tmp.x = (double)Math.round(tmp.x*100d)/100d + 0.25;
                	//tmp.y = (double)Math.round(tmp.y*100d)/100d + 0.25;
            		tmp.x += 0.20;
            		tmp.y += 0.40;
            	}
            } catch(Exception e) {
