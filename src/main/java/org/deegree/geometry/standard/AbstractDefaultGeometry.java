...
public abstract class AbstractDefaultGeometry implements Geometry {
    ...
    public boolean isWithinDistance( Geometry geometry, Measure distance ) {
    	...
    	// @@ ictway
        //JTSGeometryPair jtsGeoms = JTSGeometryPair.createCompatiblePair( this, geometry );
    	//boolean bRes = jtsGeoms.first.isWithinDistance( jtsGeoms.second, distance.getValueAsDouble() );
    	boolean bRes = false;
    	String srid = geometry.getCoordinateSystem().getAlias().toString();
    	if (srid.contains("4326")) {
    		AbstractDefaultGeometry pt1 = this;
        	Geometry pt2 = geometry;
    		
    		ICRS gcrs = CRSManager.getCRSRef("EPSG:3857");
    		GeometryTransformer transformer;
    		transformer = new GeometryTransformer(gcrs);
    		try {
				pt1 = transformer.transform(pt1, this.crs);
	    		pt2 = transformer.transform(pt2, this.crs);
			} catch (IllegalArgumentException | TransformationException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
    		
            JTSGeometryPair jtsGeoms = JTSGeometryPair.createCompatiblePair( pt1, pt2 );
        	bRes = jtsGeoms.first.isWithinDistance( jtsGeoms.second, distance.getValueAsDouble() );
    	} else {
    		JTSGeometryPair jtsGeoms = JTSGeometryPair.createCompatiblePair( this, geometry );
    		bRes = jtsGeoms.first.isWithinDistance( jtsGeoms.second, distance.getValueAsDouble() );
    	}
    ...
    public Measure getDistance( Geometry geometry, Unit requestedUnit ) {
    	...
    	if (srid.contains("4326")) {
            ...
            double dist = getJTSGeometry().distance( getAsDefaultGeometry( pt1 ).getJTSGeometry() );
            return new Measure( Double.toString( dist ), null );
    	} else {
            double dist = getJTSGeometry().distance( getAsDefaultGeometry( geometry ).getJTSGeometry() );
            return new Measure( Double.toString( dist ), null );
