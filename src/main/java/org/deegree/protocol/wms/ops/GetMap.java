...
public class GetMap extends RequestBase {
	...
	public static double[] degree2meters(double lon, double lat) {
	...
	public GetMap(Map<String, String> map, Version version, MapOptionsMaps exts) throws OWSException {
		// @@ks
		String c = (map.get("CRS")==null)?((map.get("SRS")==null)?"":map.get("SRS")):map.get("CRS");
		...
		if (c.contains(":4326")) {
			...
			else  if (version.equals(VERSION_130))
				bbox = getCRSAndEnvelope130(c, vals);

			double minx = bbox.getEnvelope().getMin().get0();
			double miny = bbox.getEnvelope().getMin().get1();
			double maxx = bbox.getEnvelope().getMax().get0();
			double maxy = bbox.getEnvelope().getMax().get1();
			
			double[] amin = degree2meters(minx, miny);
			double[] amax = degree2meters(maxx, maxy);
			
			if (version.equals(VERSION_111)) 
				map.put("SRS", "EPSG:3857");
			else  if (version.equals(VERSION_130))
				map.put("CRS", "EPSG:3857");
			
			map.put("BBOX", amin[0] +"," + amin[1] + "," + amax[0] +"," + amax[1]);
		}
		
		//LOG.info(">>>>>> Wise Sphere Info 2018.05_01 <<<<<");
		//LOG.info(">> GetMap Request BBox :" + map.get("BBOX"));
	 ...
	 * @throws OWSException
	 */
	public GetMap(Collection<LayerRef> layers, Collection<StyleRef> styles, int width, int height, Envelope boundingBox,
			MapOptionsMaps exts) throws OWSException {
	...
	private void parse111(Map<String, String> map, MapOptionsMaps exts) throws OWSException {
		...
		if (vals.length != 4) {
			throw new OWSException(
					"The bounding box parameter value " + box + " is in the wrong format (too many values).",
					OWSException.INVALID_PARAMETER_VALUE);
		}
	...
	static LinkedList<StyleRef> handleKVPStyles(String ss, int numLayers) throws OWSException {
		...
		if (ss.trim().isEmpty()) {
			...
			if (styls.length != numLayers) {
				throw new OWSException(
						styls.length + "styles have been specified, and " + numLayers + " layers (should be equal).",
						OWSException.INVALID_PARAMETER_VALUE);
			}
	...
	private void handleCommon(Map<String, String> map, MapOptionsMaps exts) throws OWSException {
		String ls = map.get("LAYERS");
		String sld = map.get("SLD");
		String sldBody = map.get("SLD_BODY");

		// BY KS
		String bs = map.get("BUFFERS");
		...
		if (!(bs == null || bs.trim().isEmpty())) {
			String[] bslist = bs.split(",");
			if (bslist.length == layers.size()) {
				for (int i = 0; i < layers.size(); i++) {
					buffers.put(layers.get(i).getName(), bslist[i]);
				}
			}
		}
	...
	private void parse130(Map<String, String> map, MapOptionsMaps exts) throws OWSException {
	...
	/**
	 * @return the KVP map of parameters. May not be accurate/empty, especially
	 *         if this object has been created by some other means than a KVP
	 *         request.
	 */
	...
	public static Envelope getCRSAndEnvelope130(String crs, double[] bbox) {
		if (crs.startsWith("AUTO2:")) {
			...
			return new GeometryFactory().createEnvelope(bbox[1], bbox[0], bbox[3], bbox[2], CRSManager.getCRSRef(crs));
		} else {
	...
	/**
	 * @return null, or a map with parameters that should be overridden when
	 *         used as client parameter object.
	 */
