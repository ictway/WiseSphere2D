...
public class Config implements Comparable<Config> {
	...
	private static final URL METADATA_EXAMPLE_URL = WebServiceConfigManager.class
			.getResource("/META-INF/schemas/services/metadata/3.2.0/example.xml");

	private static final URL METADATA_SCHEMA_URL = WebServiceConfigManager.class
			.getResource("/META-INF/schemas/services/metadata/3.2.0/metadata.xsd");
	...
	public void activate() {
		try {
			...
			// message...BY_JIN
			// FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "Unable to
			// activate resource: " + t.getMessage(), null );
			FacesMessage fm = new FacesMessage(SEVERITY_ERROR, "자원을 활성화 할 수 없습니다(오류내용: " + t.getMessage() + ")", null);
			FacesContext.getCurrentInstance().addMessage(null, fm);
			return;
		}
		state = resourceManager.getState(id);
		WorkspaceBean ws = (WorkspaceBean) FacesContext.getCurrentInstance().getExternalContext().getApplicationMap()
				.get("workspace");
		ws.setModified();
	...
	public void deactivate() {
		try {
			...
			// message...BY_JIN
			// FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "Unable to
			// deactivate resource: " + t.getMessage(), null );
			FacesMessage fm = new FacesMessage(SEVERITY_ERROR, "자원을 비활성화 할 수 없습니다(오류내용: " + t.getMessage() + ")", null);
			FacesContext.getCurrentInstance().addMessage(null, fm);
			return;
		}
		state = resourceManager.getState(id);
	...
	public void showErrors() {
		...
		// FacesMessage fm = new FacesMessage( SEVERITY_ERROR, msg, null );
		// message...BY_JIN
		//FacesMessage fm = new FacesMessage(SEVERITY_ERROR, "'" + id + "' 자원 초기화에 실패하였습니다.", null);
		FacesMessage fm = new FacesMessage(SEVERITY_ERROR, "'" + id + "' 자원 초기화에 실패하였습니다.", null);
		FacesContext.getCurrentInstance().addMessage(null, fm);
	}
		...
		try {
			...
			if (autoActivate && resourceManager != null) {
				...
				if (state.getType() == StateType.deactivated) {
					...
					if (location.getName().indexOf("theme")>-1) {
						//resourceManager.startup(OGCFrontController.getInstance().getServiceWorkspace());
						resourceManager.manualActivate(id, location);
					} else {
						resourceManager.deactivate(id);
						resourceManager.activate(id);
					}
			...
			// System.out.println(requiresWSReload);
			// requiresWSReload = true;
			// OGCFrontController fc = OGCFrontController.getInstance();
			// String wsName = fc.getServiceWorkspace().getName();
			// LOG.error(">>>>>>>>>>>>>>>>>>>>>>>>>>wsName:"+wsName);
			...
			// message...BY_JIN
			// FacesMessage fm = new FacesMessage( SEVERITY_ERROR, msg, null );
			FacesMessage fm = new FacesMessage(SEVERITY_ERROR, "변경 오류가 발생하였습니다.", null);
			FacesContext.getCurrentInstance().addMessage(null, fm);
