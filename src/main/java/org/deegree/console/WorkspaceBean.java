...
//            FileUtils.deleteDirectory( dw.getLocation() );
        	FileUtils.forceDeleteOnExit(dw.getLocation());
            
            // BY_JIN
//            File fin = new File("C:/ABC Statements final/");
            File fin = new File(dw.getLocation().toString());
...
//                throw new Exception( "Download of '" + location + "' failed. Server responded with HTTP status code " + p.second.getStatusLine().getStatusCode() );
                throw new Exception( "작업공간 '" + location + "' 다운로드가 실패하였습니다. 서버에 접속할 수 없습니다. 오류코드 " + p.second.getStatusLine().getStatusCode() );
            }
...
//            FacesMessage fm = new FacesMessage( SEVERITY_INFO, "Please select a workspace file first.", null );
            FacesMessage fm = new FacesMessage( SEVERITY_INFO, "먼저 작업공간 파일을 선택해 주십시오.", null );
            FacesContext.getCurrentInstance().addMessage( null, fm );
            return null;
        }
        ...
        // IE 파일 처리...BY_JIN
        workspaceImportName = workspaceImportName.substring(workspaceImportName.lastIndexOf("\\")+1,workspaceImportName.lastIndexOf("."));
...
//                throw new Exception( "Workspace '" + workspaceImportName + "' already exists." );
                throw new Exception( "작업공간 '" + workspaceImportName + "' 은(는) 이미 존재하는 작업공간입니다." );
            } else {
                Zip.unzip( in, target );
            }
        } catch ( Throwable t ) {
        	// message...BY_JIN
//            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "Workspace could not be imported: " + t.getMessage(), null );
            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "작업공간을 가져올 수 없습니다(오류내용: " + t.getMessage() + ")", null );
            FacesContext.getCurrentInstance().addMessage( null, fm );
            return null;
        } finally {
            closeQuietly( in );
        ...
        // message...BY_JIN
//        FacesMessage fm = new FacesMessage( SEVERITY_INFO, "Workspace '" + workspaceImportName + "' added succesfully.", null );
        FacesMessage fm = new FacesMessage( SEVERITY_INFO, "작업공간 '" + workspaceImportName + "'가 성공적으로 추가되었습니다.", null );
        FacesContext.getCurrentInstance().addMessage( null, fm );
        return WS_MAIN_VIEW;
    }
