...
//                FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "SQLDialect error: " + t.getMessage(), null );
                FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "SQLDialect 오류가 발생하였습니다(오류내용: " + t.getMessage() + ")", null );
...
//            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "At least one schema file must be selected.", null );
            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "적어도 하나의 스키마 파일이 선택되어야 합니다.", null );
            FacesContext.getCurrentInstance().addMessage( null, fm );
            return null;
        }
...
//            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, msg, t.getMessage() );
            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "GML 응용 스키마 구문분석을 할 수 없습니다.", t.getMessage() );
            FacesContext.getCurrentInstance().addMessage( null, fm );
        }
...
//                FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "Unable to create config: " + t.getMessage(), null );
            ...
            // message...BY_JIN
//            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, msg, t.getMessage() );
            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "Feature 저장소 설정을 생성할 수 없습니다.", t.getMessage() );
            FacesContext.getCurrentInstance().addMessage( null, fm );
            return "/console/featurestore/sql/wizard3";
        }
    }
    ...
    public String activateFS() {
        try {
            ...
            // BY_JIN
//            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, msg, t.getMessage() );
            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "Feature 저장소를 활성화 할 수 없습니다.", t.getMessage() );
            FacesContext.getCurrentInstance().addMessage( null, fm );
            return null;
        }
