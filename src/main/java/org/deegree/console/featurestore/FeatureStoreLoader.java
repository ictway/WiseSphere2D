...
//            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "GML import failed: " + t.getMessage(), null );
            FacesMessage fm = new FacesMessage( SEVERITY_ERROR, "GML 데이터 불러오는 중 오류가 발생하였습니다(오류내용: " + t.getMessage() + ")", null ); 
...
//        FacesMessage fm = new FacesMessage( SEVERITY_INFO, "Imported " + fids.size() + " features", null );
        FacesMessage fm = new FacesMessage( SEVERITY_INFO, fids.size() + "개 Feautre를 가져왔습니다.", null );
        FacesContext.getCurrentInstance().addMessage( null, fm );
    }
}