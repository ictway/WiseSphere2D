...
public abstract class AbstractResourceManager<T extends Resource> extends AbstractBasicResourceManager
	...
	/**
	 * Processes the given resource configuration file and returns the resulting
	 * resource state.
	 * <p>
	 ...
	 * @return resource state, can be null if resource manager does not want to
	 *         handle the config
	 * @throws IOException
	...
	public ResourceState<T> processResourceConfig(File configFile) throws IOException {
		...
		//String fileName = configFile.getAbsolutePath().substring(dirName.length());
	...
	@Override
	public ResourceState<T> manualActivate(String id, File newFile) {
		ResourceState<T> state = null;
		try {
			state = processResourceConfig(newFile);
		} catch (IOException e) {
